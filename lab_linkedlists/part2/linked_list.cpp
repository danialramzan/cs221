...
void delete_last_element(Node *&head)
{
    ...
    //case 1: list is empty after deleting element (it formerly had 1 element)

    if (head->next == nullptr) {
        delete head;
        head = nullptr;
    } else { // case 2: has >= 2 elements
...
void remove(Node *&head, int oldKey)
{
    ...
    //check if head is to be replaced.
    if (head->key == oldKey) {
        Node *temp = head;
        head = temp->next;
        delete temp;
    ...
    // general for n >= 2
        ...
        for (Node *curr = head; curr != nullptr; curr = curr->next) {
            if ((curr->next != nullptr) && (curr->next->key == oldKey)) {
                Node *temp = curr->next;
                curr->next = temp->next;
                delete temp;
                temp = nullptr;
