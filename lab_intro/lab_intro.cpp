...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned i = 0; i < image.width(); i++) {
    for (unsigned j = 0; j < image.height(); j++) {
      int distance = sqrt(((centerX - i) * (centerX - i)) + ((centerY - j) * (centerY - j)));
      if (distance > 200) {
        (*(image.getPixel(i,j))).r = 0;
        (*(image.getPixel(i,j))).g = 0;
        (*(image.getPixel(i,j))).b = 0;
      } else {
        (*(image.getPixel(i,j))).r = (*(image.getPixel(i,j))).r * (1 - (distance * 0.005));
        (*(image.getPixel(i,j))).g = (*(image.getPixel(i,j))).g * (1 - (distance * 0.005));
        (*(image.getPixel(i,j))).b = (*(image.getPixel(i,j))).b * (1 - (distance * 0.005));
      }
    }
  }
  return image;
...
PNG ubcify(PNG image) {
  ...
  for (unsigned int i = 0; i < image.width(); i++) {
     for (unsigned int j = 0; j < image.height(); j++) {
      if ((colordist(UBCYellow, *image.getPixel(i,j))) <= (colordist(UBCBlue, *image.getPixel(i,j)))) {
        *image.getPixel(i,j) = UBCYellow;
      } else {
        *image.getPixel(i,j) = UBCBlue;
      }
     }
  }
  return image;
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for (unsigned int i = 0; i < secondImage.width(); i++) {
     for (unsigned int j = 0; j < secondImage.height(); j++) {
      if (((*secondImage.getPixel(i,j)).r == 255) &
        ((*secondImage.getPixel(i,j)).g == 255) &
        ((*secondImage.getPixel(i,j)).b == 255)) {

          if (((*firstImage.getPixel(i,j)).r + 40) > 255) {
            (*firstImage.getPixel(i,j)).r = 255;
          } else {
            ((*firstImage.getPixel(i,j)).r += 40);
          }

          if (((*firstImage.getPixel(i,j)).g + 40) > 255) {
            (*firstImage.getPixel(i,j)).g = 255;
          } else {
            ((*firstImage.getPixel(i,j)).g += 40);
          }

          if (((*firstImage.getPixel(i,j)).b + 40) > 255) {
            (*firstImage.getPixel(i,j)).b = 255;
          } else {
            ((*firstImage.getPixel(i,j)).b += 40);
          }
